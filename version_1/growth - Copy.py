...
class Grain:
    ...
    rav = 0.0 #e ;            // average Radius
    mode = True #;            // calculation modes 1 
    screw = False
    h_time = 0.1 #;         // calculation step
    #g_size = np.array([]) #       // grain Radius [MAX]
    ...
    def PrintMode(self, k):
        self.c_A = k
        print(self.c_A, self.c_B, self.c_C)

    def PrintModeCallagain(self):
        self.c_D += 10
        self.c_B *= 0.01
        return self.c_A, self.c_B, self.c_D

    def MakeGrain(self, num):
        self.g_size = np.random.normal(100,10,num)
        self.g_size.sort()
        return self.g_size
