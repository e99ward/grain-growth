...
class Grain:
    ...
    def CalcControl(self, temp, stfe, liqv, ctsv, mode1, mode2):
        ...
        if stfe != self.def_sfe and mode1:
    ...
    def GetStatistics(self, final=False):
        if final == True:
            num = self.stat[-1,0]
            self.__rstat(num)
            self.SaveStatistics()
    ...
    def CalcGrowth(self, cts_num):
        ...
        for i in range(self.g_num):
            if self.g_size[i] > self.rstar:
                ...
                self.g_size[i] = 0
        self.g_size = np.trim_zeros(self.g_size)
    ...
    def SaveHistogram(self, cts_num):
        g_size = self.g_size.astype(int) 
        hist = np.bincount(g_size)
        #filename = 'h_0000000.txt.npy'
        filename = 'h_' + '{:07d}'.format(cts_num) + '.txt'
...
#--------------------------------#
# generate initial set of grains #
